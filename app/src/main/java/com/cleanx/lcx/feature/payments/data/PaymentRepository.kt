...
class PaymentRepository @Inject constructor(
    ...
    suspend fun charge(ticketId: String, amount: Double): ChargeResult {
        ...
        return when (paymentResult) {
            is PaymentResult.Cancelled -> {
                Timber.d("Payment cancelled for ticket %s", ticketId)
                ChargeResult.Cancelled(reference = ticketId)
            }

            is PaymentResult.Failed -> {
                Timber.w(
                    "Payment failed for ticket %s: %s (%s)",
                    ticketId,
    ...
    suspend fun syncPaymentToBackend(
        ...
        return when (apiResult) {
            ...
            is ApiResult.Error -> {
                Timber.e(
                    "CRITICAL: Payment succeeded (txn %s) but API PATCH failed for ticket %s: %s",
                    transactionId,
                    ticketId,
                    apiResult.message,
                )
